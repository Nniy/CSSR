...
void ParseTree::GetDataInput(char dataFile[]) {
    ...
    //create file stream
    std::ifstream inData(dataFile, std::ios::in);
...
void ParseTree::GetAlphaInput(char alphaFile[]) {
    ...
    //create file streams
    std::ifstream inAlpha(alphaFile, std::ios::in);

    //open alphabet file, if unsuccessful set boolean return value
    if (!inAlpha.is_open()) {
        std::cerr << " the alphabet file cannot be opened " << std::endl;
        exit(1);
    }

        //otherwise read in data until end of file
    else {
        inAlpha.seekg(0, std::ios::end);
        size = inAlpha.tellg();
        inAlpha.seekg(0, std::ios::beg);
...
void ParseTree::CheckAlphaInput() {
    ...
    for (int k = 0; k < m_alphaSize; k++) {
        if (!(isalnum(m_alpha[k]))) {
            std::cerr << "Alphabet contains characters which are not alphanumeric: "
                      << m_alpha[k] << "\n";
            exit(1);
        }
    }
...
void ParseTree::CheckDataInput() {
    ...
    for (int i = 0; i < m_dataSize; i++) {
        ...
        if (accept_flag)
            ...
            std::cerr << "Data contains characters which are not in the alphabet: "
                      << m_data[i] << "\n";
            exit(1);
        }
    }
...
void ParseTree::DecStringCount(char string[], TreeNode *&root) {
    ...
    if (root == NULL) {
        //no match
        std::cerr << "Attempting to decrement occurence of string which doesn't "
                  << "show up in data,  ParseTree::DecStringCount " << std::endl;
        exit(1);
    }
        //this substring already exists
    else if (root->m_symbol == string[0]) {
...
void ParseTree::ReadProcessMultiLine(char alphaFile[], char dataFile[]) {
    ...
    if (!inData.is_open()) {
        ...
        for (int k = 0; k < m_alphaSize; k++) {
            if (!(isalnum(m_alpha[k]))) {
                std::cerr << "Alphabet contains characters which are not"
                          << " alphanumeric.\n";
                exit(1);
            }
        }
        ...
        for (int i = 0; i < m_dataSize; i++) {
            ...
            if (accept_flag)
                ...
                std::cerr << "Data contains characters which are not in the"
                          << " alphabet.\n";
                exit(1);
            }
        }
...
int ParseTree::FindRoots(TreeNode *root, char charArray[], int intArray[]) {
    ...
    if (m_dataSize == 0) {
        std::cerr << "no data in tree" << std::endl;
        exit(1);
    }
    while (root != NULL) {
