...
void Machine::PrintOut(char input[], const char *alpha_file, const char *data_file, const int &max_length,
    ...
    //create file streams
    std::ofstream outData(output, std::ios::out);

    //open output file, if unsuccessful set boolean return value
    if (!outData) {
        std::cerr << " the information output file cannot be opened " << std::endl;
        exit(1);
    }
        //otherwise output data
    else {
        outData << "Alphabet File: " << alpha_file << std::endl;
        outData << "Data File: " << data_file << std::endl;
        outData << "History Length: " << max_length << std::endl;
        outData << "Significance Level: " << sigLevel << std::endl;
        outData << "Multiline Mode: " << (isMulti ? "true" : "false") << std::endl;
        outData << "Chi-squared test used: " << (isChi ? "true" : "false") << std::endl;
        outData << "Alphabet Size: " << alphaSize << std::endl;
        outData << "Relative Entropy: " << m_relEnt << std::endl;
        outData << "Relative Entropy Rate: " << m_relEntRate << std::endl;
        outData << "Statistical Complexity: " << m_cMu << std::endl;
        outData << "Entropy Rate: " << m_entRate << std::endl;
        outData << "Variation: " << m_variation << std::endl;
        ...
        if (m_allstates->getReSynch()) {
            outData << "This data needed to be synchronized to a set"
                    ...
                    << " states cannot possibly be the causal states." << std::endl;

            //Print to screen also
            std::cout << std::endl
                      << "The data in " << input << std::endl
                      << "needed to be resynchronized; try to"
                      << " increase max length." << std::endl << std::endl;
        }
...
void Machine::PrintDot(char input[], char alpha[]) {
    ...
    if (!outData) {
        std::cerr << " the .dot output file cannot be opened " << std::endl;
        exit(1);
    }
        //otherwise output data
    else {
        ...
        outData << "digraph " << input << " {" << std::endl;
        outData << "size = \"6,8.5\";" << std::endl;
        outData << "ratio = \"fill\";" << std::endl;
        outData << "node [shape = circle];" << std::endl;
        outData << "node [fontsize = 24];" << std::endl;
        outData << "edge [fontsize = 24];" << std::endl;
